...
type HeatmapValue = { date: Date; contributions: number };
...
const RandomCalendar: React.FC = () => {
  ...
  values.push({ date: new Date(), contributions: 9 });
      ...
      <div className="wrapper">
        <Heatmap
          ...
          classForValue={(item: HeatmapValue) => {
            ...
            if (item !== null) {
              clampedCount = Math.max(item.contributions, 0);
              clampedCount = Math.min(item.contributions, 4);
            }
...
const generateHeatmapValues = (startDate: Date, endDate: Date) => {
  ...
  while (isBefore(currentDate, endDate) || isSameDay(currentDate, endDate)) {
    ...
    values.push({ date: currentDate, contributions: Math.round(count) });
